...
class TestPosPreparationDisplay(TestPoSCommon):
    ...
    def test_load_preparation_display_model(self):
        ...
        self.assertEqual({d['id'] for d in config2.current_session_id.load_data([])['pos_preparation_display.display']},
                         {display1.id, display2.id, display3.id})
        self.assertEqual({d['id'] for d in config3.current_session_id.load_data([])['pos_preparation_display.display']},
                         {display1.id, display3.id, display4.id})
        self.assertEqual({d['id'] for d in config4.current_session_id.load_data([])['pos_preparation_display.display']},
