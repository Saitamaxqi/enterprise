...
class MarketingCampaignTest(TestMACommon):
    ...
    @users('user_marketing_automation')
    def test_campaign_dynamic_domain(self):
        """Check dynamic domains are effectively composed together into another dynamic domain."""
        # date domains generated by the front-end widget
        within_domain = (
            '["&", '
            '("create_date", ">=", datetime.datetime.combine(context_today(), datetime.time(0, 0, 0))'
            '.to_utc().strftime("%Y-%m-%d %H:%M:%S")),'
            '("create_date", "<=", datetime.datetime.combine(context_today() + relativedelta(days = 2), datetime.time(0, 0, 0))'
            '.to_utc().strftime("%Y-%m-%d %H:%M:%S"))'
            ']'
        )
        between_domain = (
            '["&", '
            '("create_date", ">=", datetime.datetime.combine(context_today(), datetime.time(0, 0, 0)).strftime("%Y-%m-%d %H:%M:%S")),'
            '("create_date", "<=", datetime.datetime.combine(context_today() + relativedelta(days = 2), datetime.time(0, 0, 0)).strftime("%Y-%m-%d %H:%M:%S"))'
            ']'
        )
        simple_domain = '[("id", "!=", 1 + 1)]'
        campaign = self.env['marketing.campaign'].create({
            'domain': within_domain,
            'name': 'Domain Parent',
        })
        campaign.marketing_activity_ids = [(0, 0, {
            'activity_domain': between_domain,
            'campaign_id': campaign.id,
            'child_ids': [
                (0, 0, {
                    'activity_domain': simple_domain,
                    'campaign_id': campaign.id,
                    'name': 'Grand-child',
        ...
        self.assertEqual(campaign.marketing_activity_ids.child_ids.domain, (
            "['&', '&', ('id', '!=', 1 + 1), '&', "
            "('create_date', '>=', datetime.datetime.combine(context_today(), datetime.time(0, 0, 0))"
            ".to_utc().strftime('%Y-%m-%d %H:%M:%S')), "
            "('create_date', '<=', datetime.datetime.combine(context_today() + relativedelta(days=2), datetime.time(0, 0, 0))"
        ...
        # check that it can be executed successfully without traceback
